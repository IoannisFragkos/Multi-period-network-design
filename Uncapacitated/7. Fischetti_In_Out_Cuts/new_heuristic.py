...
def make_model(data, fixed_cost):
    ...
    for arc in xrange(arcs):
        ...
        for h in xrange(commodities):
            ...
            flow[h, arc] = model.addVar(
                obj=variable_cost[arc] * data.demand[0, h] * demand_factor,
                lb=0., ub=min(
                    1., capacity[arc] / data.demand[0, h] * demand_factor),
    ...
    for arc in xrange(arcs):
        ...
        capacities[arc] = model.addConstr(grb.quicksum(grb.LinExpr(
            data.demand[0, h] * demand_factor, flow[h, arc]) for h in xrange(
...
def make_local_branching_model(data, kappa, open_arcs):
    """
...
def heuristic(data, kappa, mode='u'):
