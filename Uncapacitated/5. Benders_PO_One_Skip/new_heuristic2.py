...
def make_local_branching_model(data, open_arcs, cutoff):
    ...
    for arc in xrange(arcs):
        model.addSOS(
            type=grb.GRB.SOS_TYPE1, vars=arc_open[:, arc].tolist(), wts=weights)
        # model.addConstr(
...
def solve_local_branching_model(data, open_arcs, lb_model, cutoff):
    ...
    for arc in xrange(arcs):
        if not arcs_in_search[arc] and open_arcs[:, arc].sum() > 0.5:
            max_period = open_arcs[:, arc].argmax()
            if max_period < 3:
                end = min(periods, max_period + 9)
                lhs = grb.quicksum([
    ...
    if lb_model.numConstrs > 0:
        constraint = lb_model.getConstrs()[lb_model.numConstrs - 1]
        lb_model.remove(constraint)

    expr = grb.LinExpr(
        [-1.] * len(arcs_one) + [1.] * len(arcs_zero), arcs_one + arcs_zero)
    lb_model.addConstr(lhs=expr, rhs=6, sense=grb.GRB.LESS_EQUAL, name='lb')
    ...
    if lb_model.objVal < cutoff:
        ...
        open_arcs = np.array(
            lb_model.getAttr('x', arc_open.flatten().tolist())).reshape(
            periods, arcs)
        lb_model.setAttr(
            'Start', arc_open.flatten().tolist(), open_arcs.flatten().tolist())
...
def heuristic(data, kappa, mode='u', local_branching=False):
