...
def main():
    ...
    subproblems, subproblems_po = populate_dual_subproblem(data)
    master = populate_master(data)
    master_callback = callback_data(subproblems, data, subproblems_po)
...
def populate_dual_subproblem(data, upper_cost=None, flow_cost=None):
    ...
    for period, com in product(periods, commodities):
        if (period, com) != (0, 0):
            ...
            dual_subproblem_po.ModelName = 'dual_subproblem_po({},{})'.format(period, com)
            all_vars = np.array(dual_subproblem_po.getVars())
            ubounds_duals_po = all_vars.take(ubounds_index)
            flow_duals_po = all_vars.take(flow_index)
            obj = LinExpr(flow_duals_po[origins[com]] - flow_duals_po[destinations[com]])
            obj.addTerms([-0.99]*len(arcs), ubounds_duals_po.tolist())
            dual_subproblem_po.setObjective(obj, GRB.MAXIMIZE)
            dual_subproblem_po._all_variables = all_vars
...
def callback_data(subproblems, data, subproblems_po):
    """
...
#    @profile
        ...
        for period in periods:
            for com in commodities:
                ...
                if status_arr[period, com] == GRB.status.OPTIMAL:
                    ...
                    constr_basis = subproblem.getAttr('CBasis', subproblem.getConstrs())
                    ...
                    if po_obj1 > 10e-3:
                        ...
                        lhs = LinExpr(flow_duals[data.origins[com]] - flow_duals[data.destinations[com]])
                        lhs.addTerms(sum_setup[period, :].tolist(), ubound_duals.tolist())
                        constr = subproblem.addConstr(lhs >= opt_val - 10e-4, name='po({},{})'.format(period, com))
                        new_obj = LinExpr(flow_duals[data.origins[com]] - flow_duals[data.destinations[com]])
                        obj_coeffs = np.copy(sum_setup[period, :])
                        obj_coeffs[obj_coeffs.nonzero()[0]] = -1. + EPSILON # * np.round(np.random.rand(), 1)
                        obj_coeffs[np.where(obj_coeffs == 0)[0]] = - EPSILON # * np.round(np.random.rand(), 1)
                        new_obj.addTerms(obj_coeffs.tolist(), ubound_duals.tolist())
                        subproblem.setObjective(new_obj, sense=GRB.MAXIMIZE)
                        all_variables = all_variables.tolist()
                        # New: warm-start basis
                        subproblem.setAttr('VBasis', all_variables, var_basis)
                        subproblem.setAttr('CBasis', subproblem.getConstrs(), constr_basis)
                        subproblem.optimize()
                        # print 'Itercount is {}'.format(subproblem.IterCount)
                        all_duals = np.array(subproblem.getAttr("X", all_variables))
                        ubound_duals_vals = all_duals.take(ubound_index)
                        flow_duals_vals = all_duals.take(flow_index)
                        subproblem.remove(constr)
                        subproblem.update()
                elif status_arr[period, com] in (4, 5):
                    all_variables = np.array(subproblem.getAttr('UnbdRay', subproblem.getVars()))
    ...
    def master_callback(model, where):
        if where == GRB.callback.MIPSOL:
            ...
            if (node_count % 1000 == 0 or node_count < 10000) and model.cbGet(
