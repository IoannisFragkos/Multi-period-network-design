                ...
                for com in xrange(data.commodities):
                    ...
                    if subproblem_status == GRB.status.OPTIMAL:
                        ...
                        lhs = populate_benders_cut(duals, model,
                                                   period, com, data)
                        # viol = get_viol(model, lhs)
                        # if abs(viol) - abs(max_viol) > 10e-5:
                        #     max_viol = viol
                        #     max_lhs = lhs
                        model.cbLazy(
...
def get_viol(master, lhs):
    viol = lhs.getConstant()
    vars = [lhs.getVar(i) for i in xrange(lhs.size())]
    coeffs = [lhs.getCoeff(i) for i in xrange(lhs.size())]
    vals = master.cbGetSolution(vars)
    viol += sum(x * y for x, y in zip(coeffs, vals))
    return min(viol, 0.)
